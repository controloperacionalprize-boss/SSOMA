...
# ── Playwright ────────────────────────────────────────────────────────────────
async def _async_pdf(html_str: str, footer: str) -> bytes:
    ...
    async with async_playwright() as p:
        ...
        pdf = await page.pdf(
            ...
            margin={"top": "8mm", "bottom": "16mm", "left": "5mm", "right": "5mm"},
        )
        await browser.close()
    return pdf


def generar_pdf(html_str: str, footer: str) -> bytes:
    if sys.platform == "win32":
        asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(_async_pdf(html_str, footer))
    finally:
        loop.close()
...
with col_b1:
    if st.button("🔍 Generar Vista Previa", use_container_width=True, type="secondary"):
        with st.spinner("Generando informe..."):
            pdf_bytes = generar_pdf(build_html_pdf(), FOOTER_TEMPLATE)
